"""
    plot_thermalization_halo.jl

Visualiza T_th vs Δκ con efecto de halo (varianza como gradiente de color alrededor de puntos).
Calcula σ(T_th) desde E_optical de cada realización en el ensamble.

Estilo: Plots.jl con scatter, fuentes grandes, efecto visual elegante.

Usage:
  julia --project=. examples/plot_thermalization_halo.jl results/data/ensemble_production_100real/ensemble_results_YYYY-MM-DD.jld2
"""
...
function plot_halo_scatter!(p, deltas, means, stds, color_main, color_halo; n_layers=5)
    ...
    Parámetros:
      - deltas: valores X (Δκ)
      - means: valores Y centrales (T_th medio)
      - stds: desviaciones estándar (σ(T_th))
      - color_main: color central del punto
      - color_halo: color del halo
      - n_layers: número de capas concéntricas del halo
    """

    # Capas del halo (de más transparente a menos)
    for layer in 1:n_layers
        alpha_halo = 0.25 * (1.0 - (layer - 1) / n_layers)  # alpha decreciente
        markersize_layer = 6 + layer * 2.5  # tamaño creciente
...
function main()
    if isempty(ARGS)
        println("Usage: julia examples/plot_thermalization_halo.jl <results.jld2>")
        return
    end
    ...
    for result in results
        ...
        # Calcular T_th para cada realización
        ...
        push!(E_opt_stds, 0.0)  # por ahora solo media
    end
    ...
    # --- Figura 1: T_th vs Δκ (escala log) con halo de varianza ---
    p1 = plot(
    ...
    savefig(p1, "results/figures/ensemble/thermalization_time_halo_log.pdf")
    println("Saved: results/figures/ensemble/thermalization_time_halo_log.pdf")
    ...
    savefig(p2, "results/figures/ensemble/thermalization_time_halo_linear.pdf")
    println("Saved: results/figures/ensemble/thermalization_time_halo_linear.pdf")
    ...
    savefig(p3, "results/figures/ensemble/optical_energy_halo.pdf")
    println("Saved: results/figures/ensemble/optical_energy_halo.pdf")
    ...
    p4a = plot(
        ...
        lw=2.5, legend=false, grid=true, yscale=:log10,
        framestyle=:box, bottom_margin=2Plots.mm, left_margin=5Plots.mm
    )
    ...
    p4b = plot(
        xlabel=latexstring("\\Delta\\kappa"),
        ylabel=latexstring("E_{\\mathrm{opt}}/E_{\\mathrm{tot}}"),
        lw=2.5, legend=false, grid=true, ylim=(0.2, 0.6),
        framestyle=:box, bottom_margin=5Plots.mm, left_margin=5Plots.mm
    )
    ...
    savefig(p_combined, "results/figures/ensemble/thermalization_halo_combined.pdf")
    println("Saved: results/figures/ensemble/thermalization_halo_combined.pdf")
